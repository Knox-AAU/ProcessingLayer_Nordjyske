...
class TestGetNearestDocument (unittest.TestCase):
    def test_get_nearest_document_2d(self):
        ...
        expected_output = [
            {'id': 1, 'vec': [[2, math.sqrt(((10-1)**2)+((9-2)**2)+((8-3)**2))]]},
            {'id': 2, 'vec': [[1, math.sqrt(((1-10)**2)+((2-9)**2)+((3-8)**2))]]}
        ]

        self.assertEqual(get_neareast_arts(article_vector_list), expected_output)
    ...
    def test_get_nearest_document_2d_zero(self):
        ...
        expected_output = [{'id': 1, 'vec': []}, {'id': 2, 'vec': []}]
    ...
    def test_get_nearest_document_2d_normalize(self):
        ...
        expected_output = [
            {'id': 1, 'vec':
                [[2, math.sqrt(((0.5659-0.2672)**2)+((0.1616-0.5345)**2)+((0.8084-0.8017)**2))]]},
            {'id': 2, 'vec':
    ...
    def test_get_nearest_document_3d(self):
        ...
        expected_output = [
            {'id': 1, 'vec': [[2, math.sqrt(((5-1)**2)+((1-2)**2)+((4-3)**2))],
                [4, math.sqrt(((10-1)**2)+((9-2)**2)+((8-3)**2))]]},
            {'id': 4, 'vec': [[2, math.sqrt(((5-10)**2)+((1-9)**2)+((4-8)**2))],
                [1, math.sqrt(((1-10)**2)+((2-9)**2)+((3-8)**2))]]},
            {'id': 2, 'vec': [[1, math.sqrt(((1-5)**2)+((2-1)**2)+((3-4)**2))],
